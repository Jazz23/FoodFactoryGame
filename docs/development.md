...
- `Factory3DTraversalController` is enabled for the local production player by `GameSceneManager.EnsureLocalProduction3DPlayer`; the prototype enables it directly. It owns cloned Input System actions, uses a `CharacterController` in the prototype or a child capsule collider in production, and maps the stable X/Z foot anchor through `FactorySpatialAdapter` to logical X/Y. It permits planar X/Z movement only; floor index and elevation are runtime-derived presentation data.
- `Set3DOwnership(false)` returns simulation to the existing `Movement` and `Rigidbody2D`; the controller does not replace or rewrite logical state. UI pointer/text focus, build context, and active transitions block traversal input. `PlayerSceneTransition.Request3DInteraction` forwards portal/elevator requests to the existing server/scene seams rather than loading or saving directly.
