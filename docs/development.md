...
- Company cash is shown top right of the HUD. Nothing earns or spends it yet.
- World save cost (decision 0012): a hosting or server-only process logs `[Goods] commits=<n> avg=<ms> max=<ms> payload=<KB>` once a minute, cumulative for the process, and a one-time `[Goods]` warning if a commit takes over 50 ms or the payload passes 1 MB. Those thresholds are the signals for moving the world from the JSON payload to relational tables. In the Editor the counters also include test saves made since the last domain reload, so take measurements from a player build or a fresh Editor session.
