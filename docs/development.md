...
Customer choice benchmark (prototype, test-only; no customer system exists): `run_tests` with `mode: editor`, `filter_type: assembly`, `filter: FoodFactoryGame.Benchmarks.EditModeTests`; require 6 matched/6 passed. Each scenario logs one `[Benchmark]` console line (machine, mean/p99/max tick, outcome counts). Budgets are provisional and Editor Mono timings are not server timings. Unity's Mono always reports 0 from `GC.GetAllocatedBytesForCurrentThread`, so check allocations with `UnityEngine.TestTools.Constraints.Is.Not.AllocatingGCMemory()`. Results: [the benchmark record](verification/customer-choice-benchmark-20260925.md).
