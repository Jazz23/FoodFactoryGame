...
- Open `Assets/Scenes/Factory3DPrototype.unity` to evaluate the reversible 3D slice. It uses a two-floor in-memory fixture, adapter-based logical picking, the reusable capsule-based `Factory3DTraversalController`, and discrete floor cycling through the existing Build/Rotate action.
...
## Unified 3D Traversal Seam

- `Factory3DTraversalController` is enabled for the local production player by `GameSceneManager.EnsureLocalProduction3DPlayer`; the prototype enables it directly. It owns cloned Input System actions, uses a `CharacterController` in the prototype or a child capsule collider in production, and maps the stable orthogonal X/Z foot anchor through `FactorySpatialAdapter` to logical X/Y. It permits planar X/Z movement only; floor index and elevation are runtime-derived presentation data.
- `Set3DOwnership(false)` returns simulation to the existing `Movement` and `Rigidbody2D`; the controller does not replace or rewrite logical state. UI pointer/text focus, active build placement, and active transitions block traversal input; merely having build tools available does not stop traversal. `PlayerSceneTransition.Request3DInteraction` forwards portal/elevator requests to the existing server/scene seams rather than loading or saving directly.
- `Factory3DTraversalCollisionPresenter` can derive collision-only floor, wall, and equipment geometry from a read-only route snapshot. Existing disposable proxy assemblers continue to remove render colliders so their current presentation contracts remain unchanged.
- Focused seam coverage is `Factory3DTraversalTests` (adapter foot anchor, grounded capsule collision, input ownership/UI gating, and 3D portal reachability). It is an isolated EditMode suite and does not open the application database.
- Production hookup is complete without scene YAML edits: `Player.prefab` was authored through Unity's prefab API with `Factory3DTraversalController` and its child capsule, `GameSceneManager` creates the disposable OutsideTest collision presenter, and local ownership is selected after network ownership is established. `PlayerTest.prefab` remains the legacy 2D test fixture.

## Runtime 3D Presentation Bridge

- `Factory3DPresentationBridge` is created by `GameSceneManager` at runtime, so the conversion does not add generated proxy identities or Unity transforms to authoring assets or saves. It waits for an initialized `NAIStateManager` and a valid OutsideTest `SceneGrid`, then rebuilds the disposable shell proxy from authoritative building/floor records.
- While 3D presentation is active, the bridge captures and disables world `SpriteRenderer`, `TilemapRenderer`, `Collider2D`, and `Rigidbody2D` components. It excludes screen-space Canvas descendants and restores every captured enabled/simulated value on authority loss, scene loss, disable, or destruction. The existing route, interior, collision, construction, and simulation presenters remain separate owners.
- The bridge creates disposable capsule visuals for objects carrying `Factory3DTraversalController`, hides their legacy sprite presentation, and configures the active enabled camera across loaded scenes. Bootstrap-owned cameras are supported; the calibrated symmetric dimetric camera uses 45-degree yaw, 30-degree downward elevation, and the `sqrt(0.5)` orthographic-size scale derived from the existing 2D diamond basis. Camera transform/projection and `CameraFollow` ownership are restored when the bridge clears.
- Focused coverage is `Factory3DPresentationBridgeTests` (4 EditMode tests): reversible legacy gating with UI protection, authority-loss cleanup, player visual lifecycle, fixed-angle camera restore, and Bootstrap-camera resolution. The suite uses temporary editor scenes and never opens or writes `factory-world.db`.
...
- `Factory3DConstructionController` is runtime-facing and is auto-created by `GameSceneManager` for the loaded `OutsideTest` scene. Its mutually exclusive modes are `Idle`, `BuildingPlacement`, `EquipmentPlacement`, `MovingSelection`, and `RemovalConfirmation`; equipment previews are generated only in equipment mode, and removal selection creates a fresh target-specific confirmation without committing. Public `BeginBuildingPlacement`, `BeginEquipmentPlacement`, `BeginSelectedBuildingMove`, `BeginSelectedEntityMove`, `TryCommitCurrentPreview`, `TryRemoveSelection`, `TryConfirmRemoval`, and `CancelConstruction` APIs are suitable for UI/tooling. It clones the `Player` and `Build` Input System maps, owns an explicit construction camera, blocks pointer-focused UI input, and coordinates active 3D ownership with `FactoryBuildController`.
...
- Focused construction coverage is `FactoryConstructionServiceTests` (18 EditMode cases) plus `Factory3DOutsideTestProxyTests` (15 EditMode cases): orthogonal/dimetric 3D picking, serialized/runtime scene bounds, footprint/default-door/overlap validation, equipment/conveyor bounds and occupancy, elevator alignment, authoritative interior-only semantics, explicit input ownership/UI focus, placement/move/confirmed-removal flow, stable identities, top-story roof derivation, reversible active-floor occlusion, route-root preservation, proxy reconciliation, and committed isolated save/load gameplay-state persistence. Deferred milestone items are arbitrary rotation, free camera, and NavMesh.

## Runtime 3D Construction Verification

- `FactoryConstructionServiceTests` is the focused EditMode construction suite. It uses in-memory authority plus unique temporary SQLite paths, and removes `-wal`/`-shm` sidecars; it never targets `factory-world.db`.
- The focused command is `unity command run_tests --project-path E:\Projects\Unity\FoodFactoryGame --mode editor --filter FactoryConstructionServiceTests --filter_type testName`; a successful run must report a nonzero match count and no failures. The broader `Factory3D` EditMode filter covers proxy, interior, route, and lifecycle presentation regressions.
- The named `factory_verify` `playmode` profile is the runtime smoke check. Record its run ID, requested empty filter, matched count, summary, and JSON/XML artifact paths; zero matches and infrastructure failures are not acceptance evidence. A failed prototype physics-startup assertion must be reproduced with the isolated test-name filter before changing construction code or adding waits.
- Construction acceptance covers equipment preview creation, mode/ownership/UI gating, fresh removal confirmation, authoritative interior offsets, both grid projections and presentation scales, placement/move/remove preservation, isolated save/reload, and proxy cleanup on authority loss. Production art, free camera, ramps, and NavMesh remain deferred until this loop is stable.
