...
- `Factory3DOutsideTestProxyAssembler` adds a derived roof only to the top story. `Factory3DOutsideTestProxyView.SetActiveFloor` provides a reversible inspection mode that isolates one floor and hides that floor's roof to avoid presentation occlusion; clearing the mode restores the full shell.
