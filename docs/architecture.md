...
Rules are in [decision 0004](decisions/0004-station-job-rules.md). This is domain code only. There is **no** network command, placed station, `OvenToggle` display, visuals, or recipe authoring yet.
...
- `RemoveStationDurably` models machine pickup. It returns a running job's inputs, or a blocked job's output, to a same-site carried location, then removes the job and the station. It refuses with `capacity` if they don't fit, `invalid-route` for another site or the station's own buffers, and `forbidden` without a grant. The station's buffer locations and their lots are left untouched, pending step 2.
...
These remain accepted contracts; only the bounded goods slice and its station-job domain above have a runtime interface.
