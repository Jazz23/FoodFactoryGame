...
Prototype: the dev shell's size, position, doorway and materials. Open: construction and purchase of land/shells (section 5), wall and door editing, interior walls, extra floors, floor area in any rule (customers, seating), reserving doorways, and whether indoors should affect server rules.
...
These remain accepted contracts; only the bounded goods slice, its station jobs, equipment placement, the working oven, conveyor belts, company cash, the sell counter, supplier purchases, equipment purchases, spoilage timing/refrigeration and building shells above have a runtime interface. Sales credit cash inside the clock tick; purchases are the one player payment command.
...
- SQLite is the storage for all persisted data (decision 0011): the goods world, player registry and client identity. MoonSharp remains a declared dependency with no selected role.
