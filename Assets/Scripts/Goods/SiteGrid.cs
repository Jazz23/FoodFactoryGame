...
namespace FoodFactoryGame.Goods
{
    public static class SiteGrid
    {
        ...
        public static string PlacementProblem(GoodsSnapshot state, GoodsEquipment equipment, int cellX, int cellZ, int rotation)
        {
            ...
            var layout = state.SiteLayouts?.FirstOrDefault(x => x.SiteId == equipment.SiteId);
            ...
            foreach (var other in state.Equipment.Where(x => x.Id != equipment.Id && x.SiteId == equipment.SiteId && x.State == EquipmentState.Placed))
            {
