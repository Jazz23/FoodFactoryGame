...
namespace FoodFactoryGame.Goods
{
    ...
    public sealed partial class GoodsWorld
    {
        ...
        public GoodsOutcome PlaceBelt(string playerId, string requestId, string siteId, int cellX, int cellZ, int direction)
        {
            lock (_gate)
            {
                ...
                var source = _state.Lots
                    .Where(x => x.LocationId == inventory.Id && x.ItemId == BeltItemId && x.OwnerId == siteId && Available(x) > 0)
                    .OrderBy(x => x.Id, StringComparer.Ordinal).FirstOrDefault();
                if (source == null) return Record(requestId, playerId, false, "no-belts", null);
        ...
        // The belt item and every item riding the belt go to the player's inventory together, or nothing changes.
        public GoodsOutcome RemoveBelt(string playerId, string requestId, string beltId)
        {
            lock (_gate)
            {
                ...
                {
                    Id = Guid.NewGuid().ToString("N"), ItemId = BeltItemId, OwnerId = belt.SiteId, LocationId = inventory.Id,
                ...
                var stack = _state.Lots.Where(x => x.LocationId == inventory.Id && x.ItemId == BeltItemId && x.OwnerId == belt.SiteId
